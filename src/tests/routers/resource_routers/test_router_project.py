...
@pytest.mark.parametrize("cost_field", ["total_cost_euro", "total_cost_euros"])
def test_total_cost_euro_forward_compatible(
        cost_field: str,
        client: TestClient,
        body_resource: dict,
        auto_publish: None,
    ...
    assert response.json()["total_cost_euro"] == 10000000.53
    assert response.json()["total_cost_euros"] == 10000000.53
