...
BASE_FIELDS = ["{0}.identifier", "{0}.name", "text.plain as 'plain'",
               "text.html as 'html'", "aiod_entry.date_modified"]
...
def main():
    ...
    generate_pipeline_conf_files(pipeline_conf_path, db_user, db_pass,
                                 es_user, es_pass, entities.keys(), sync=False)
    generate_pipeline_conf_files(pipeline_conf_path, db_user, db_pass,
                                 es_user, es_pass, entities.keys(), sync=True)
    
    # Generate SQL init, sync and rm files
    for entity, fields in entities.items():
        generate_pipeline_sql_files(pipeline_sql_path, entity, fields,
                                    sync=False)
        generate_pipeline_sql_files(pipeline_sql_path, entity, fields,
                                    sync=True)
