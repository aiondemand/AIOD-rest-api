...
def resource_read(resource_class: Type["AIoDConcept"] | Type["Platform"]) -> Type[SQLModel]:
    ...
    # What is the `Field()` is it the OneToOne, e.g.? -> FieldInfo
    # generated by us in relationships.py's `_ResourceRelationship`, result from calling Field(...)
    ...
    # Create the Pydantic model, note that resource_class.__base__ ensures propagation of 'simple'
    # types that don't have a relationship, such as `is_accessible_for_free` or `legal_name`.
    # Why is the AIoDConcept's `identifier` not included but its `date_deleted` is?
    # `date_deleted` isn't, `date_published` is. This is because `resource_class.__base__` doesn't
    # inherit from `AIoDConcept` (the relationship side) but form `AIoDConceptBase` (the simple
    # attribute side). As to why `identifier` lives here and not on Base, I am not sure.
    # Perhaps to simplify exactly this kind of logic.
    model = create_model(
    ...
    if resource_class.__name__ == 'Organisation':
        breakpoint()
    return model
